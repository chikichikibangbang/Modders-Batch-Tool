...
from ...utils.materials import korone
...
def _get_image(mat: bpy.types.Material) -> Union[bpy.types.Image, None]:
    ...
    #node = node_tree.nodes.get(shader_image_nodes.get(shader, ''))
    node = korone(mat)
...
def _set_image_or_color(item: StructureItem, mat: bpy.types.Material) -> None:
    if globs.is_blender_2_80_or_newer:
        ...
        #node_tree = mat.node_tree if mat else None
        node = korone(mat)
        item['gfx']['img_or_color'] = get_packed_file(node.image) if node else None 
